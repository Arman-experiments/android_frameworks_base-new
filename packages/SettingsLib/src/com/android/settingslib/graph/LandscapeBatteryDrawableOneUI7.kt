...
open class LandscapeBatteryDrawableOneUI7(private val context: Context, frameColor: Int) : Drawable() {
    ...
    override fun draw(c: Canvas) {
        ...
        // Only center the text when not in special states
        val pctX: Float
        val pctY: Float
        
        // Show off-center position with bolt for both charging and power save mode
        if ((charging || powerSaveEnabled) && batteryLevel < 100) {
            // Keep the original positioning when charging or in power save mode
            // Calculate position in one step to avoid reassigning val
            pctX = (bounds.width() + textHeight) * 0.7f * 0.8f  // Equivalent to * 0.7f and then * 0.8f
            pctY = bounds.height() * 0.8f
        } else {
            // Center the text when in normal state
            pctX = bounds.width() * 0.5f  // Center horizontally
            pctY = bounds.height() * 0.5f - ((textPaint.descent() + textPaint.ascent()) / 2)  // Center vertically
        }
        ...
        // Add bolt to the path when charging or in power save mode
        ...
        if (charging && animationActive) {
            ...
            if (elapsedTime < ANIMATION_DURATION_MS) {
                ...
                val glowAlpha = ((Math.sin(animationTime) + 1) / 2 * 155 + 100).toInt()
    ...
    private val FRAME_RATE_MS: Long = 50 // Animation frame rate in milliseconds
